...
def main():
    ...
    sample_userID = '7517052' # (howardlindzon)  # Start with sample user
    sample_df = df_with_text[df_with_text['user_id_str'] == sample_userID]
    sample_df_rt = df_with_text[df_with_text['rt_user_id'] == int(sample_userID)]
    sample_df_rt = sample_df_rt.drop_duplicates(subset='rt_id_str', keep='first')

    #sample_df = sample_df.sample(n=min(sample_size, len(df_with_text)), random_state=42)
